   ...
   const onUploadImage = (e: React.ChangeEvent<HTMLInputElement>) => {
	...
	return (
      ...
      <div className="flex gap-6 items-center">
        <div>
          <Image className="rounded-full aspect-square" src={avatarUrl} width={64} height={64} alt="dammy"/>
        </div>
        <div>
          <input hidden ref={inputRef} type="file" id="avatar"onChange={onUploadImage} />
          <button onClick={onProfileButtonClick} className="inline-flex h-10 items-center gap-2 text-sm rounded-full border border-slate-300/50 px-4 hover:bg-slate-100 transition duration-300">プロフィール画像を変更</button>
        </div>
      </div>
      <p className="text-sm text-red-500 mt-4">{fileMessage}</p>
    </div>
	)
}
